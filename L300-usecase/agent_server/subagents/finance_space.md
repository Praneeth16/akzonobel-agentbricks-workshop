...
> against `${AKZO_CATALOG}.akzo_finance.*`. Do not invent columns — every column below exists in
...
All tables fully-qualified under `${AKZO_CATALOG}.akzo_finance`.

| Table | Purpose | Key columns | Grain |
|---|---|---|---|
| `${AKZO_CATALOG}.akzo_finance.products` | SKU master / product dimension | `sku`, `product_name`, `product_line` (Decorative Paints \| Performance Coatings), `region` (EMEA/Americas/APAC/China), `currency`, `list_price_eur`, `standard_cost_eur` | one row per SKU |
| `${AKZO_CATALOG}.akzo_finance.margin_actuals` | Realized monthly P&L per SKU×region | `sku`, `region`, `month` (DATE, first-of-month), `units`, `revenue_eur`, `cogs_eur`, `gross_margin_eur`, `gross_margin_pct` (fraction, 0.42 = 42%) | sku × region × month |
| `${AKZO_CATALOG}.akzo_finance.margin_budget` | Plan / budget figures (no shocks) | `sku`, `region`, `month`, `budget_units`, `budget_revenue_eur`, `budget_margin_eur` | sku × region × month |
| `${AKZO_CATALOG}.akzo_finance.fx_rates` | Monthly FX to EUR | `currency` (EUR/USD/GBP/CNY), `month`, `rate_to_eur` (EUR=1.0) | currency × month |
| `${AKZO_CATALOG}.akzo_finance.cost_drivers` | COGS decomposition | `sku`, `region`, `month`, `raw_material_cost`, `freight_cost`, `energy_cost`, `overhead` (EUR; sum reconciles to `cogs_eur`) | sku × region × month |
...
WITH base AS (
  ...
  FROM ${AKZO_CATALOG}.akzo_finance.margin_actuals m
  JOIN ${AKZO_CATALOG}.akzo_finance.products p ON m.sku = p.sku
  LEFT JOIN ${AKZO_CATALOG}.akzo_finance.cost_drivers c
...
SELECT
  ...
  ROUND(SUM(gross_margin_eur) / SUM(revenue_eur) * 100, 1) AS gross_margin_pct
FROM ${AKZO_CATALOG}.akzo_finance.margin_actuals
...
FROM ${AKZO_CATALOG}.akzo_finance.cost_drivers c
JOIN ${AKZO_CATALOG}.akzo_finance.products p ON c.sku = p.sku
WHERE p.product_line = 'Decorative Paints' AND p.region = 'EMEA'
...
FROM ${AKZO_CATALOG}.akzo_finance.fx_rates
...
FROM ${AKZO_CATALOG}.akzo_finance.margin_actuals m
JOIN ${AKZO_CATALOG}.akzo_finance.products p ON m.sku = p.sku
JOIN ${AKZO_CATALOG}.akzo_finance.margin_budget b
