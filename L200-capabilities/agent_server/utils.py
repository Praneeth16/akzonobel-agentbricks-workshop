"""Helpers for the L200 OpenAI Agents SDK agent.

Everything that touches a workspace is read from the environment, so the same
code runs unchanged on a laptop and on a Vocareum lab. Nothing here hardcodes a
catalog, workspace URL, warehouse, or Lakebase instance.
"""
import logging
import os
...
class LakebaseConfig:
    """Lakebase connection, populated from env. Supports a provisioned instance
    (LAKEBASE_INSTANCE_NAME) or an autoscaling endpoint/project+branch."""
...
def init_lakebase_config() -> Optional[LakebaseConfig]:
    """Read Lakebase config from env. Returns None (memory disabled) if unset."""
...
def build_mcp_url(path: str, workspace_client: WorkspaceClient | None = None) -> str:
    """Turn a relative MCP path (e.g. /api/2.0/mcp/functions/<cat>/<schema>) into
    a full URL using the workspace host. Absolute URLs are returned unchanged."""
